...
G4bool MySensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory* ROhist)
{
    ...
    // to avoid the different values, like entering from the side. (for the scintillator below one was uncommented). 
    // we have coomented it out for the TOF, otherwise it kills the particle ass soon as it hits the first scintillator tile. 
    ...
    // here below pre, post or may be average. All are same as time windows or tiles are pretty small.
    G4double time = preStepPoint->GetGlobalTime(); // either local time which starts when particle is created or global time which starts at the start of new run.
    // here global and local times are same as particle is created at the start of the run. However, during decays say they are differernt
