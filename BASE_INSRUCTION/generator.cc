...
// we have defined particle as radioactive decay as long as we have not set it in the macro file.
...
MyPrimaryGenerator::MyPrimaryGenerator()
{
    ...
    // for the cobalt thingy.
    //G4ThreeVector pos(0.,0.,0.); //create particle at the origin
    //G4ThreeVector mom(0.,0.,1.); //momentum along the z direction

    // for isAtmosphere, that is for the showers. we need to place the proton where it is initially created, that is on the board of our volume.
    G4ThreeVector pos(0.,0.,-20.*km);
    G4ThreeVector mom(0.,0.,1.);
    // for creating proton go into the vis.mac.

    fParticleGun -> SetParticlePosition(pos);
    fParticleGun -> SetParticleMomentumDirection(mom);
    fParticleGun -> SetParticleMomentum(100.*GeV); //magnitude of the momentum high energy = 100GeV. If zero, we can set it in macro files, otherwise at this value it will be radioactive decay of the cobalt source.
